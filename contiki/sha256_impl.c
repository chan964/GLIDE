/*
 * sha256_impl.c — Minimal SHA-256 implementation.
 * Based on public domain FIPS 180-4 reference implementation.
 */
...
static void sha256_transform(sha256_impl_ctx_t *ctx, const uint8_t *data)
{
...
void sha256_impl_final(sha256_impl_ctx_t *ctx, uint8_t *digest)
{
    ...
    uint8_t bits_buf[8];
    for(i=7;i>=0;i--) { bits_buf[i]=(uint8_t)(bits&0xff); bits>>=8; }
    sha256_impl_update(ctx, bits_buf, 8);
...
void sha256_impl_hash(const uint8_t *data, size_t len, uint8_t *digest)
{
    ...
    sha256_impl_update(&ctx, data, len);
    sha256_impl_final(&ctx, digest);
}
